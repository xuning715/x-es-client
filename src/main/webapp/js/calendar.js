...
function writeIframe()
{
    ...
    ".out { text-align: center; border-top: 1px solid "+ WebCalendar.lightColor +"; border-left: 1px solid "+ WebCalendar.lightColor +";border-right: 1px solid "+ WebCalendar.lightColor +"; border-bottom: 1px solid "+ WebCalendar.lightColor +";}"+
    ...
    "case 84 : document.forms[0].today.click(); break;} window.event.returnValue= false;}"+
    ...
    "    <td width=60 id=meizzYearHead  title='点击此处选择年份' bgColor='#9ec3fb' onclick='parent.funYearSelect(parseInt(this.innerText, 10))'"+
    "></td>"+
    "    <td width=50 id=meizzYearMonth title='点击此处选择月份' bgColor='#9ec3fb' onclick='parent.funMonthSelect(parseInt(this.innerText, 10))'"+
    "></td>"+
    ...
    "    <tr align=center><td height=20 class=out>日</td><td class=out>一</td><td class=out>二</td><td class=out>三</td><td class=out>四</td><td class=out>五</td><td class=out>六</td></tr></table>"+
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=0 cellpadding=0>";
         for(var x=0; x<5; x++){ strIframe += "<tr align=center>";
    ...
    " onfocus='this.blur()' name=prevMonth title='向前翻 1 月&#13;快捷键：←' onclick='parent.prevM()' type=button value='&lt;'><input name=today type=button value='今天 ' onfocus='this.blur()' title='今天&#13;快捷键：T' onclick=\"parent.returnDate(new Date().getDate() +'/'+ (new Date().getMonth() +1) +'/'+ new Date().getFullYear())\"><input type=button value='清空' onclick=\"parent.returnClear()\"><input title='向后翻 1 月&#13;快捷键：→' name=nextMonth onclick='parent.nextM()' type=button value='&gt;' onfocus='this.blur()'><input name=nextYear title='向后翻 1 年&#13;快捷键：↓' onclick='parent.nextY()' type=button value='&gt;&gt;' onfocus='this.blur()' style='meizz:expression(this.disabled=parent.WebCalendar.thisYear==9999)'></td></tr></table>"+
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.darkColor  = "#9ec3fb";    //控件的暗色
    this.lightColor = "#ffffff";    //控件的亮色
    this.btnBgColor = "#9ec3fb";    //控件的按钮背景色
    this.wordColor  = "#ffffff";    //控件的文字颜色
    this.wordDark   = "#7591bc";    //控件的暗文字颜色
    this.dayBgColor = "#9ec3fb";    //日期数字背景色
    this.todayColor = "#6699FF";    //今天在日历上的标示背景色
    this.DarkBorder = "#9ec3fb";    //日期显示的立体表达色
...
function funYearSelect() //年份的下拉框
{
    ...
    var min = 1900;//(y - n >= 1000) ? y - n : 1000;
    var max = 2050;//(y + n <= 9999) ? y + n : 9999;
...
function returnClear() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
...
function dealString(date,format){
  //date = '' + date;
...
function showDate(string){
	 ...
	 if(len == 8){
		 date = string.substring(0,4)
			    + "/"
			    + string.substring(4,6)
			    + "/"
			    + string.substring(6,len);
	 }
	 else if(len == 6){
		 date = string.substring(0,4)
			    + "/"
		        + string.substring(4,len);
	 }
	 else{
		 date = string;
	 }
...
function showTodayDate(){
	var year   = new Date().getFullYear(); //定义年的变量的初始值
    var month  = new Date().getMonth() + 1; //定义月的变量的初始值
    var day    = new Date().getDate();     //定义日的变量的初始值
